...
# Known Issues:
# time delay between Computer due to wireless connection
...
def phrasing_audio_direction():
    ...
    while determine_direction: 
        ...
        if "forward" or "for" in text_lst_direction:
...
def phrasing_audio_distance(): #with string input because sphinx audio couldn't or is hard to recieve numerical audio input
    # text_lst_distance = ""
    distance = int(input('distance 1 - 3 meter'))
    # determine_distance = True
    # max_distance = 3 #declare max distance
...
def move():
        ...
        while True :
                ...
                distance_moved = (abs(0.5 * math.sqrt(((x-x0) ** 2) + ((y-y0) ** 2))))*2 # times 4 for real robot
